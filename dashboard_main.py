...
def line_plot_base(input_df: pd.DataFrame, yintercept_val: int, shortfall_val: int):
    ...
    fig.update_layout(
        margin=dict(l=10, r=10, t=50, b=10), yaxis_title=None, xaxis_title=None
    )
...
def spending_line_plot(profit_target: int, historical_start_year: int):
    ...
    monthly_income = Finances_Dashboard_Helpers.get_monthly_income(
        finance_df, most_recent_obs.month - 1, most_recent_obs.year
    )
...
def monthly_finance_barchart(
    month: int, year: int, profit_target: int, housing_payment: int
):

    filter_df = Finances_Dashboard_Helpers.create_spend_budget_df(
        finance_df, budget_df, year, month, housing_payment, profit_target
    )
...
def avg_category_piechart(housing_payment: int, historical_start_year: int):
    ...
    avg_df = (
        historical_df.groupby(["year", "month", "category"])
        .agg({"total": "sum"})
        .groupby(["category"])
        .agg({"total": "mean"})
        .reset_index(drop=False)
    ...
    housing_adder = (
        housing_payment - avg_df[avg_df["category"] == "housing"]["total"].sum()
    )
    avg_savings = (
        graph_df[graph_df["category"] == "saving"]["total"].mean() + housing_adder
    )
    ...
    avg_df = pd.concat(
        [avg_df, pd.DataFrame(data={"category": ["savings"], "total": [avg_savings]})]
    )
...
def accounts_table(
    ...
    avg_spend_df = avg_spend_df[
        ~avg_spend_df["category"].isin(["loans", "housing", "bonus"])
    ]

    monthly_df = (
        avg_spend_df[avg_spend_df["category"].isin(["income", "investments"])]
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    html.Div([dcc.Graph(id="avg_category_piechart")]), width={"size": 6}
                ),
            ],
            justify="evenly",
        ),
                ...
                dbc.Col(
                    html.Div(
                        [
                            ...
                            dcc.Input(
                                id="historical_start_year", value=2019, type="number"
                            ),
                        ]
                    ),
                    style={"textAlign": "right"},
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    html.Div([dcc.Graph(id="savings_line_plot")]), width={"size": 5}
                ),
                dbc.Col(
                    html.Div([dcc.Graph(id="spending_line_plot")]), width={"size": 5}
                ),
            ],
            justify="evenly",
            align="center",
